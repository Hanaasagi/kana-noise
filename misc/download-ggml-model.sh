...
# get the path of this script (POSIX)
get_script_path() {
...
# Default download dir: project_root/models/whisper
project_root="$(cd "$script_path/.." >/dev/null 2>&1 || exit ; pwd -P)"
default_models_dir="$project_root/models/whisper"
mkdir -p "$default_models_dir"

# Allow overriding via second arg
models_path="${2:-$default_models_dir}"
mkdir -p "$models_path"
